...
void read_evaluation_words()
{
  long long i = 0;
  char *line;
  ssize_t read, len = 0;
  printf("\nReading evaluation words from %s ...\n", evaluation_file);
  FILE *fin = fopen(evaluation_file, "rb");
  if (fin == NULL)
  {
    printf("Evaluation file not found\n");
    exit(1);
  }
  ...
  while ((read = getline(&line, &len, fin)) != -1)
  {
    ...
    printf("%s, ", line);
    strcpy(evaluation_words[i], line);
    i++;
  }
  eval_size = i;
  printf("\nThe number of evaluation words: %llu\n", eval_size);
  fclose(fin);
}
...
void InitialModel()
{
  ...
  LearnVocabFromTrainFile(); //if (read_vocab_file[0] != 0) ReadVocab(); else LearnVocabFromTrainFile();
...
void SaveWordVectors(int with_target, char *target_word, int only_target_word)
{
  long a, b;
  FILE *fo;
  ...
  if (only_target_word)
  {
    a = SearchVocab(target_word);
    fprintf(fo, "target-%s ", vocab[a].word);
    if (hs)
      for (b = 0; b < layer1_size; b++)
    ...
    fprintf(fo, "\n");
    fprintf(fo, "context-%s ", vocab[a].word);
    for (b = 0; b < layer1_size; b++)
...
void SaveEvaluationVectors(int with_target, char *output_file)
{
  long a, b;
  FILE *fo;
  char *target_word;
  ...
  {
    a = SearchVocab(evaluation_words[i]);
...
void TrainModelWithSerial()
{
  ...
  {
    target_word_index = SearchVocab(evaluation_words[eval_word_count]);
    printf("\nThe target word is %s and its size is %d, its index is %d", evaluation_words[eval_word_count], strlen(evaluation_words[eval_word_count]), target_word_index);
    ...
    while (1)
    {
      ...
      if (sentence_length == 0 && (feof(fi) || (word_count > train_words)))
      {
        word_count_actual += word_count - last_word_count;
        local_iter--;
        if (local_iter == 0)
  ...
  {
    printf("\nword vectors are saving...\n");
    SaveEvaluationVectors(1, output_file); //SaveWordVectors(1, target_word, 1);
  }
...
void *TrainModelThread(void *id)
{
  ...
  while (1)
  {
    ...
    if (sentence_length == 0 && (feof(fi) || (word_count > train_words / num_threads)))
    {
      word_count_actual += word_count - last_word_count;
      local_iter--;
      if (local_iter == 0)
...
void TrainModel()
{
  ...
  if (evaluation_file[0] != 0)
  {
    char output[MAX_STRING];
    strcpy(output, output_file);
    strcat(output, "-original");
    SaveEvaluationVectors(1, output);
    printf("\nThe prior file name: %s  ---   the org is %s \n", output_file, output);
    return;
  }
...
int main(int argc, char **argv)
{
  ...
  if ((i = ArgPos((char *)"-target-word", argc, argv)) > 0)
    strcpy(target_word, argv[i + 1]);
  if ((i = ArgPos((char *)"-gpu", argc, argv)) > 0)
    gpu = atoi(argv[i + 1]);
  if ((i = ArgPos((char *)"-eval-file", argc, argv)) > 0)
    strcpy(evaluation_file, argv[i + 1]);
