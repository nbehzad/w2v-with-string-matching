...
void SaveWordVectors(int with_target, char *target_word, int only_target_word){
    ...
    if (only_target_word){
		...
		if (hs) 
			for (b = 0; b < layer1_size; b++) fprintf(fo, "%lf ", syn1[a * layer1_size + b]); 
		else
  ...
  for (a = 0; a < iter; a++) {
	  ...
	  for (unsigned long i = 0; i < params.text_size; i++) {	
		if (params.match[i] == 1) {
			count++;
			c_index = 0;
			for (b = 0; b < window * 2; b++) context_word_list[b] = -1;
			/*
			if(0){
			...
			next_random = next_random * (unsigned long long)25214903917 + 11;
			for (b = 0; b < 2 * window; b++) if (context_word_list[b] != -1) {
				l1 = context_word_list[b] * layer1_size;
				for (c = 0; c < layer1_size; c++) neu1e[c] = 0;
				// HIERARCHICAL SOFTMAX
				if (hs) for (d = 0; d < vocab[target_word_index].codelen; d++) {
				  f = 0;
				...
				if (negative > 0) for (d = 0; d < negative + 1; d++) {
				  if (d == 0) {
					...
					if (target == target_word_index) continue;
					label = 0;
				  }
...
void TrainModelWithSerial(char *target_word) {
  InitialModel();
  start = clock();
